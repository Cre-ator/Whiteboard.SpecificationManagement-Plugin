...
class specmanagement_database_api
{
   ...
   {
      $query = "DROP TABLE mantis_plugin_DocumentManagement_src_table";

      $this->mysqli->query( $query );

      $query = "DROP TABLE mantis_plugin_DocumentManagement_type_table";

      $this->mysqli->query( $query );

      $query = "DROP TABLE mantis_plugin_DocumentManagement_ptime_table";

      $this->mysqli->query( $query );

      $query = "DROP TABLE mantis_plugin_DocumentManagement_vers_table";

      $this->mysqli->query( $query );

      $query = "DELETE FROM mantis_config_table
          WHERE config_id LIKE 'plugin_DocumentManagement%'";
   ...
   private function get_mantis_plugin_table( $table )
   {
      if ( $this->get_mantis_version() == '1.2.' )
      {
         $mantis_plugin_table = plugin_table( $table, 'DocumentManagement' );
      }
      else
      {
         $mantis_plugin_table = db_get_table( 'plugin_DocumentManagement_' . $table );
      }
